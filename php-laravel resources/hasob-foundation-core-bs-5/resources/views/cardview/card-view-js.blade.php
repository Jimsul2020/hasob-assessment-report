<script type="text/javascript">
    $(document).ready(function() {
        ...
        function {{ $control_id }}_display_results(endpoint_url) {
            ...
            $('#{{ $control_id }}-div-card-view').append(
                ...
                if (response != null && response.paginate && response.result_count > 0) {
                    ...
                    for (let pg = 1; pg <= response.pages_total; pg++) {
                        $("#{{ $control_id }}-pagination").append(
                            `<li class="page-item"><a data-val='${pg}' data-type='pg' class='{{ $control_id }}-pg pg-${pg} page-link' href='#'>${pg}</a></li>`
                        );
        ...
        $(document).on('keyup', "#{{ $control_id }}-txt-search", function(e) {
            ...
            {{ $control_id }}_display_results("{{ $control_obj->getJSONDataRouteName() }}?st=" +
                search_term);
        });
        ...
        $(document).on('click', ".{{ $control_id }}-grp", function(e) {
            ...
            {{ $control_id }}_display_results("{{ $control_obj->getJSONDataRouteName() }}?grp=" +
                group_term);
